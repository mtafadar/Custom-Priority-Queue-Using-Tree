...
private:
    ...
    void printInorder (NODE* r, ostream& output) {
        if ( r != nullptr) {
            printInorder(r->left, output);
            output  <<r-> priority<< " value: " << r->value << endl;
            printInorder(r->link, output);
            printInorder(r -> right, output);
        }
    }
    void _clear (NODE* r) {
        if (r == nullptr) {
